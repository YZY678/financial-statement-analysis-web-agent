"""
专业财经Agent核心调度器 - 修复21个错误版
修复：未解析引用、未使用导入、静态方法、参数问题
版本：3.1
"""
import os
import time
...
try:
    ...
    def create_professional_chart(df: pd.DataFrame, chart_type: str, title: str, output_dir: str = "./charts"):
        return None, "工具模块不可用"


    def analyze_financial_health(df: pd.DataFrame, report_type: str):
        return {"error": "工具模块不可用"}


    def detect_financial_report_type(df: pd.DataFrame):
        return {"type": "unknown"}
...
class FinanceAgent:
    ...
    def _ensure_directories():
        """确保必要的目录存在"""
        directories = ["./charts", "./data", "./reports", "./output/charts", "./output/reports"]
        if CHART_DIR:
            directories.append(str(CHART_DIR))
    ...
    def _execute_generate_chart(self, tool_params: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            base_title = tool_params.get("title", "财经数据分析")
            output_dir = tool_params.get("output_dir") or (str(CHART_DIR) if CHART_DIR else "./charts/")

            # 🆕 获取源文件名（不带扩展名）
            source_name = None
            if self.state["current_file"]:
                source_name = os.path.splitext(os.path.basename(self.state["current_file"]))[0]
            ...
            for chart_type in chart_types:
                ...
                chart_path, error = create_professional_chart(
                    data_to_use,  # 参数1: df
                    chart_type,  # 参数2: chart_type
                    chart_title,  # 参数3: title
                    output_dir  # 参数4: output_dir (可选，有默认值)
                )
    ...
    def _execute_analyze_health(self, tool_params: Dict) -> Dict[str, Any]:
        """执行财务健康分析 - 修复版"""
        try:
            # 检查数据可用性
            if not self.state["data_loaded"] or self.state["current_data"] is None:
            ...
            if data_to_use is None or (hasattr(data_to_use, 'empty') and data_to_use.empty):
                return {
                    "status": "error",
                    "response": "没有可用的数据，无法进行分析"
                }

            # 获取报表类型
            report_type = tool_params.get("report_type", "general")
            if report_type == "从上下文获取" and self.state["report_type"]:
                report_type = self.state["report_type"]

            # 分析财务健康
            health_report = analyze_financial_health(data_to_use, report_type)

            # 格式化结果
            result_parts = ["📈 财务健康分析报告"]

            if health_report.get("key_metrics"):
                result_parts.append("🔢 关键指标:")
                for metric, value in health_report["key_metrics"].items():
                    if metric == "profit_margin":
                        result_parts.append(f"   利润率: {value}%")
                    elif metric == "debt_ratio":
                        result_parts.append(f"   资产负债率: {value}%")
                    else:
                        result_parts.append(f"   {metric}: {value}")

            if health_report.get("warnings"):
                result_parts.append("⚠️  风险提示:")
                for warning in health_report["warnings"]:
                    result_parts.append(f"   • {warning}")

            if health_report.get("recommendations"):
                result_parts.append("💡 改进建议:")
                for rec in health_report["recommendations"]:
                    result_parts.append(f"   • {rec}")

            if not health_report.get("key_metrics") and not health_report.get("warnings"):
                result_parts.append("📊 已进行基本数据分析")
            ...
            if "ambiguous" in error_msg or "truth value" in error_msg:
                return {
                    "status": "error",
                    "response": "❌ 财务分析出错：数据处理逻辑问题，建议检查数据格式"
                }
            return {
                "status": "error",
                "response": f"财务分析异常: {error_msg[:100]}"
            }
    ...
    def _generate_chart_title(df: pd.DataFrame, chart_type: str, base_title: str) -> str:
        ...
        chart_type_names = {
            "income_trend": "趋势分析",
            "profit_composition": "构成分析",
            "balance_sheet": "资产负债分析",
            "revenue_comparison": "对比分析",
            "expense_breakdown": "费用分析"
        }

        chart_type_name = chart_type_names.get(chart_type, "分析")
        ...
        if numeric_cols:
            ...
            if len(numeric_cols) <= 3:
                data_desc = "、".join(numeric_cols[:3])
            else:
                data_desc = f"{numeric_cols[0]}等{len(numeric_cols)}项数据"
        else:
            data_desc = "财经数据"

        return f"{data_desc}{chart_type_name}图表"
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "workflow_status": {
                ...
                "workflow_history": len(self.state["workflow_history"]),
                "last_chart": self.state.get("last_chart_type")
            }
        }
    ...
    def reset(self, reset_type: str = "soft") -> Dict[str, Any]:
        ...
        print(f"🔄 Agent状态已重置 ({reset_type})")
        return {"status": "success", "message": f"Agent已{reset_type}重置"}
    ...
    def export_session_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(report_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"   报表类型: {self.state['report_type'] or '未知'}\n\n")
