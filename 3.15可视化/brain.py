...
class Brain:
    ...
    def _setup_tool_knowledge(self) -> None:
        """设置工具知识库"""
        self.tool_knowledge = {
            'load_financial_data': {  # ✅ 更名为 load_financial_data
            ...
            'detect_financial_report_type': {  # ✅ 修正函数名
    ...
    def _plan_execution(self, intent_analysis: Dict, tools_description: List[Dict]) -> Dict[str, Any]:
        ...
        if primary_intent == 'upload':
            plan.update({
                'need_tool': True,
                'tool_name': 'load_financial_data',  # ✅ 修改为实际可用的工具
                'workflow': ['load_financial_data']
            })
            ...
            plan.update({
                ...
                'workflow': ['load_financial_data', 'detect_financial_report_type']  # ✅ 添加报表类型检测
            })

        elif primary_intent == 'clean':
            plan.update({
                'need_tool': True,
                'tool_name': 'clean_financial_data',  # ✅ 工具名不变
        ...
        # 图表生成
            ...
            plan.update({
                'need_tool': True,
                'tool_name': 'create_professional_chart',  # ✅ 工具名不变
                'tool_params': {
        ...
        # ❌ 删除财务分析部分（因为 tools.py 中已删除 analyze_financial_health）
        # 状态查询
    ...
    def _build_chart_workflow(self) -> List[str]:
        ...
        if not state.get('data_loaded'):
            workflow.append('load_financial_data')

        # 检查报表类型是否已检测
        if not state.get('report_type'):
            workflow.append('detect_financial_report_type')
    ...
    def _generate_status_report(self) -> str:
        ...
        if self.context['workflow_history']:
            ...
            action_names = {
                'load_financial_data': '加载文件',
                'clean_financial_data': '清洗数据',
                'detect_financial_report_type': '识别报表类型',
                'create_professional_chart': '生成图表',
                'parse_finance_file': '解析文件'  # 保留向后兼容
            }
            recent_names = []
